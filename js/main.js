...
function saveFiles( obj ) {
  ...
  // 新建目录
  if(!fs.existsSync(saveDir+'/'+projName)) { 
    fs.mkdirSync(saveDir+'/'+projName); 
  }
  saveDir = saveDir+'/'+projName;
  // 新建目录
  if(!fs.existsSync(saveDir+'/'+imgDir)) { 
    fs.mkdirSync(saveDir+'/'+imgDir); 
  }
...
function ftpUpload( sourceDir, targetDir ) {
  var Ftp = new JSFtp(config.ftpHost)
  Ftp.raw.mkd(targetDir + '/' + sourceDir.match(/\/?([^\/]*)$/)[1], function(err, data) {
    // if (err) throw err;
    Ftp.destroy();
    Ftp.raw.quit(function(err, data) { if (err) throw err; });
...
$('#test').on('click', function() {
  var Ftp = new JSFtp(config.ftpHost);
  Ftp.ls("/newforward/static", function(err, res) {
